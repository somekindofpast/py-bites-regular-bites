...
def basic_series() -> pd.Series:
    ...
    """
    return pd.Series(data = [1, 2, 3, 4, 5], dtype = "int64", name = "Fred")
...
def float_series() -> pd.Series:
    ...
    """
    return pd.Series(data = [float(i * 0.001) for i in range(0, 1001)])
...
def alpha_index_series() -> pd.Series:
    ...
    """
    return pd.Series(data = [i for i in range(1, 27)], dtype = "int64", index = [c for c in string.ascii_lowercase])
...
def object_values_series() -> pd.Series:
    ...
    Don't worry about the series name.
    """
    return pd.Series(data = [c for c in string.ascii_uppercase], dtype = "object", index = [i for i in range(101, 127)])