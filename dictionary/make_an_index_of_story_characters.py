...
"""
characters_neetle_tree = ('elder brother', 'younger brother', ('the tree', 'magical tree'))
...
def make_character_index(text=text, characters=CHARACTERS):
    ...
    for i in range(len(lines)):
        for synonyms in characters:
            name = name_occurrence_in_line(synonyms, lines[i])
            if name != '':
                character_index[name].append(i + 1)
    return character_index

def name_occurrence_in_line(synonyms, line: str) -> str:
    if isinstance(synonyms, str):
        if synonyms.lower() in line.lower():
