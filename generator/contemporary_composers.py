...
def _get_date(date_str) -> datetime.date:
...
def operas_both_at_premiere(guest, composer):
    ...
    """
    if guest.lower() not in composers.keys() or composer.lower() not in composers.keys():
        raise ValueError("Unknown guest / composer")

    composer_obj: Composer = composers[composer.lower()]
    composer_born = _get_date(composer_obj.born)
    composer_died = _get_date(composer_obj.died)

    guest_obj: Composer = composers[guest.lower()]
    guest_born = _get_date(guest_obj.born)
    guest_died = _get_date(guest_obj.died)

    return (opera.play for opera in operas if
            opera.author == composer.lower() and
            composer_born < _get_date(opera.date) <= composer_died and
            guest_born < _get_date(opera.date) <= guest_died)


if __name__ == '__main__':
    wagner_verdi = list(operas_both_at_premiere("wagner", "verdi"))
    print(len(wagner_verdi), wagner_verdi)
    beethoven_wagner = list(operas_both_at_premiere("beethoven", "wagner"))
    print(len(beethoven_wagner), beethoven_wagner)