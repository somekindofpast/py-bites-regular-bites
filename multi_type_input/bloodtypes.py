...
def check_bt(donor, recipient):
    ...
    """
    _validate_input(donor)
    _validate_input(recipient)
    ...
    for value in _particular_antigen_comp(donor, recipient):
        if value < 0:
            return False
    return True


def _validate_input(input_val):
    ...
    """
    if not isinstance(input_val, int) and not isinstance(input_val, str) and not isinstance(input_val, Bloodtype):
        raise TypeError
    if isinstance(input_val, int) and not (0 <= input_val <= 7):
        raise ValueError
    if isinstance(input_val, str) and input_val not in blood_type_text.keys():
        raise ValueError
