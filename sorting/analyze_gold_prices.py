...
def years_gold_value_decreased(gold_prices: str = gold_prices) -> Tuple[int, int]:
    ...
    """
    price_changes = defaultdict(float)
    gold_prices = gold_prices.split()
    for i in range(1, len(gold_prices)):
        prev_year_price = float(gold_prices[i-1].split(',')[1])
        this_year_price = float(gold_prices[i].split(',')[1])
        price_changes[int(gold_prices[i].split(',')[0].split('-')[0])] = this_year_price - prev_year_price
    price_changes = sorted(price_changes.items(), key=lambda x: x[1])
    return price_changes[0][0], price_changes[-1][0]
