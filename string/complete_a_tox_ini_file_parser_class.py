...
class ToxIniParser:
    ...
    def environments(self):
        ...
        envlist = []
        for line in self.config["tox"]["envlist"].splitlines():
            line = line.strip()
            if ',' in line:
                items = line.split(',')
                envlist.extend([item.strip() for item in items if item != ''])
            elif line != '':
                envlist.append(line)
        return envlist
    ...
    def base_python_versions(self):
        ...
        base_pys = set()
        for name, value in self.config.items():
            if "basepython" in self.config[name]:
                base_pys.add(self.config[name]["basepython"])
...
if __name__ == '__main__':
    ...
    tempfile = TMP / 'ini_file'

    for ini_file in ini_files:
        with open(tempfile, 'wb') as f:
            f.write(ini_file.encode())

        tip = ToxIniParser(Path(tempfile).resolve())

        print("number_of_sections:", tip.number_of_sections)
        print("environments:", tip.environments)
        print("base_python_versions:", tip.base_python_versions)
        print("-------------------------")