...
def sysinfo_scrape(output: str):
    """Scrapes the output from screenfetch and returns a dictionary"""
    scraped_info = {}
    for line in output.splitlines():
        match_obj = re.search(r" {2,}([a-zA-Z]+ ?[a-zA-Z]+): (.+)", line)
        if match_obj:
            scraped_info[match_obj.group(1)] = match_obj.group(2)
        else:
            match_obj = re.search(r" {2,}(?P<Name>\S+@\S+)", line)
            if match_obj:
                scraped_info.update(match_obj.groupdict())
    return scraped_info
